...
| B7 | Не реализовано | DataSource техдолг: external/computed/websocket/mock/sqlite + серверный feed-polling/webhook | LOW | OPEN |
...
- Severity: LOW. Статус: OPEN (бэклог). Зависит от B6.
- Заблокированы в UI (`status:'techdebt'`), создать нельзя: `external`
  (WordPress/Strapi/Shopify), `computed` (трансформация других источников),
  `websocket`, `mock`, `rest` (legacy-алиас rest-api).
- `database`: только PostgreSQL + MySQL; `sqlite` → техдолг (нужен
  `better-sqlite3`, нативный модуль).
- `feed`: серверный планировщик (cron) и webhook-pull (`webhookSecret`) НЕ
  реализованы — авто-обновление работает только как клиентский polling. Нужны
  `node-cron`/очередь + воркер (single-point-of-failure, согласовано отложить).
- Ограничение параметров database: `:name` внутри строкового литерала SQL будет
  ошибочно заменён (документировано в коде; не использовать двоеточие-имя в
  кавычках).
