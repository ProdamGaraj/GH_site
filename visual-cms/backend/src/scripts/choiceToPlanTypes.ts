 ...
 *
 * Тело комментария описано как `(?:(?!\*\/)[\s\S])*` — «любой символ, с
 * которого не начинается закрывающая последовательность». Обычный ленивый
 * `[\s\S]*?` здесь опасен: не найдя `if` после ближайшего `*​/`, он тянется до
 * следующего, и совпадение начинает захватывать всё между двумя разными
 * комментариями. На реальном скрипте блока это вырезало ~30 КБ кода.
 */
